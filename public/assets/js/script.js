...
Steps.buildParseUrl = function() {
  ...
  $('#parse-url').html(url + '/parse/btg_database');
}
...
ParseRequest.postData = function() {
  ...
  XHR.POST('/parse/btg_database/classes/GameScore');
}
...
ParseRequest.getData = function() {
  ...
  XHR.GET('/parse/btg_database/classes/GameScore');
}
...
ParseRequest.postCloudCodeData = function() {
  ...
  XHR.POST('/parse/btg_database/functions/hello');
}
...
XHR.POST = function(path, callback) {
  ...
  this.xhttp.setRequestHeader("X-Parse-Application-Id", "UUR4D6YV5SFHDFGI");
